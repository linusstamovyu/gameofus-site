// PLACEHOLDER: flat colours so the picker works. Replaced by the neon theme build.
import { Ground } from "../map";
import { shadow, signBoard } from "./kit";
import type { Theme } from "./types";

const BAND = ['#140f24', '#1d1b2b', '#1b2231', '#1c2030', '#3a3d4d', '#121420'];
...
export const neon: Theme = {
  ...
  stageBg: "#0a0714",
  swatch: ["#e0379b", "#0a0714"],
  lines: { barrier: "Traffic’s too fast", blocked: "Can’t walk there" },
  tile(g, band, x, y, T) {
  ...
  tall(ctx, T, x, y) {
    ...
    ctx.fillStyle = "#e0379b"; ctx.fillRect(cx - T * 0.3, by - T * 2, T * 0.6, T * 1.9);
  },
  small(ctx, T, x, y) {
    const cx = x * T + T / 2, by = (y + 1) * T;
    shadow(ctx, cx, by - T * 0.12, T * 0.35, T * 0.12);
    ctx.fillStyle = "#e0379b"; ctx.fillRect(cx - T * 0.3, by - T * 0.8, T * 0.6, T * 0.7);
  },
