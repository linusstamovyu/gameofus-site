...
// the way the live pricing ladder shows it on the home page.
import { el } from "../../dom";
...
export const editionStep: StepView = (ctx, panel) => {
  ...
  for (const id of EDITION_IDS) {
    ...
    const card = el("div", `edition${chosen ? " on" : ""}${tier.star ? " star" : ""}`);
    const head = el("span", "ed-head");
    head.append(el("b", null, tier.name), el("span", "tag", copy.tag));
    ...
    if (ladder.display === "perPersonFirst") {
      ...
      price.append(el("span", "ed-total", ed.people > 3 ? `≈ ${money(ctx, shown / ed.people, true)} per friend` : "For two or three"));
    }
    const fits = el("span", "ed-fit");
    const extra = Math.max(0, d.friends.length - ed.includes.characters);
